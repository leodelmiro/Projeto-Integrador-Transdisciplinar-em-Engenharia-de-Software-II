...
class PedidoCriacaoDTO(
    ...
    companion object {
        fun PedidoCriacaoDTO.toEntidade(usuarioRepository: UsuarioRepository, produtoService: ProdutoService) =
                Pedido(
                        status = Status.valueOf(status),
                        valor = valorTotalPedido(produtoService),
                        code = UUID.randomUUID().toString(), //Código de Barra Fake
                        usuario = usuarioRepository.findById(this.usuarioId)
                                .orElseThrow { RecursoNotFoundException("Id ${this.usuarioId} de usuário não encontrado") },
                        produtos = this.produtos.map { produto -> produtoService.encontrarEntidadePorId(produto.id) },
